...
const validationSchema = Yup.object({
  ...
  agreeToTwoGenders: Yup.boolean()
    .oneOf([true], "You must agree to this")
    .required("You must agree to this")
});
...
const SignUp = () => {
    ...
    onSubmit: async (values) => {
      try {
        const response = await axios.post(
          "http://your-api-endpoint.com/auth/register", // Update with your server's endpoint
          values
        );
  ...
  const checkUsernameExists = async () => {
    ...
    try {
      const response = await axios.post(
        `http://your-api-endpoint.com/auth/check-username/${formik.values.username.toLowerCase()}` // Update with your server's endpoint
      );
  ...
  const checkEmailExists = async () => {
    ...
    try {
      const response = await axios.post(
        `http://your-api-endpoint.com/auth/check-email/${formik.values.email.toLowerCase()}` // Update with your server's endpoint
      );
      if (response.data.exists) {
        setEmailError("Email already in use");
      } else {
  ...
  const checkPasswordStrength = (password) => {
    ...
    const requirementsMet = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]/.test(
      password
    );

    let progressBarColor = "#f50057"; // Default: red
    if (passwordStrength >= 3 && requirementsMet) {
      progressBarColor = "#ffc400"; // Medium: amber
    }
    if (passwordStrength >= 4 && requirementsMet) {
      progressBarColor = "#4caf50"; // Strong: green
    }
    if (passwordStrength === 6) {
      progressBarColor = "#1976d2"; // Very Strong: blue
    }
